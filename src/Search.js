  ...
  return (
    <div>
      <form onSubmit={(event) => { event.preventDefault(); props.getUser(); props.getRepos() }}>
        ...
        <input style={{ backgroundColor: props.colorinput, color: props.colorp2 }} type="search"
          value={props.user} onChange={(event) => { props.setUser(event.target.value) }}
          className='input' onClick={() => { defultuser() }} />
      </form>
